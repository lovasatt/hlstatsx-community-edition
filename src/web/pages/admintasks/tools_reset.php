...
<?php
    ...
    if (isset($_POST['confirm']))
    {
        ...
        {
            $game_esc = $db->escape(trim((string)$_POST['game']));
            $gamefilter = " WHERE game='$game_esc'";
        }
        ...
        if (isset($_POST['clear_pcounts']) || $clearAll || $clearAllDelete)
        {
            ...
            $SQL = "UPDATE hlstats_Players SET connection_time=0, kills=0, deaths=0, suicides=0, teamkills=0, shots=0, hits=0, headshots=0, last_skill_change=0, kill_streak=0, death_streak=0 $gamefilter";
            if ($db->query($SQL))
            {
                echo "OK</li>\n";
            }
    ...
    else
    {
        ...
        $games = array();
        $games[] = '<option value="" selected="selected">All games</option>';
...
<table width="350" border="0" style="margin:auto;"><tr class="bg1"><td class="fNormal" align="left">
...
<strong>Note</strong> You should <a href="<?php echo htmlspecialchars($g_options['scripturl'] ?? '', ENT_QUOTES, 'UTF-8') . "?mode=admin&amp;task=tools_perlcontrol"; ?>" style="text-decoration:underline;font-weight:bold">stop the HLX:CE daemon</a> before resetting the stats. You can restart it after the reset completes.<br /><br />

<input type="hidden" name="confirm" value="1" />

<input type="submit" value="  Click here to confirm Reset  " class="submit" />
</td>
        </tr>
