...
&nbsp;&nbsp;&nbsp;&nbsp;<img src="<?php echo IMAGE_PATH; ?>/downarrow.gif" alt=""><strong>&nbsp;<?php echo htmlspecialchars($task->title); ?></strong>
...
<?php
   ...
   $commands[1]["cmd"] = "KILL";
	...
	$host = isset($_POST['masterserver']) ? $_POST['masterserver'] : '127.0.0.1';
	$port = isset($_POST['port']) ? (int)$_POST['port'] : 27500;
        
        $cmd_idx = isset($_POST["command"]) ? $_POST["command"] : 0;
	$command = isset($commands[$cmd_idx]["cmd"]) ? $commands[$cmd_idx]["cmd"] : false;
	...
	{
            $proxy_key = isset($g_options['Proxy_Key']) ? $g_options['Proxy_Key'] : "";
	    if ($proxy_key == "") 
	    {
	...
	echo "<li>Sending Command to HLstatsX: CE Daemon at " . htmlspecialchars($host) . ":" . $port . " &mdash; ";
	...
	$socket = socket_create(AF_INET, SOCK_DGRAM, SOL_UDP);
	    ...
	    $packet = "";
	    if (isset($g_options['Proxy_Key']) && $g_options['Proxy_Key'])
	    {
	    ...
	    if ($packets>0) {
		echo "<li>Backend Answer: ".htmlspecialchars($answer)."</li>";
	    } 
	    else 
	    {
		echo "<li><em>No packets received &mdash; check if backend dead or not listening on ".htmlspecialchars($host).":$port</em></li>";
	    }
	...
	echo "<img src=\"".IMAGE_PATH."/rightarrow.gif\" alt=\"\" /> <a href=\"{$g_options['scripturl']}?mode=admin\">Return to Administration Center</a>";
	}
	else
	{
