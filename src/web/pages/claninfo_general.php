...
<div class="subblock">
    <div style="float:left;vertical-align:top;width:48.5%;">
        <table class="data-table">
            ...
            <tr class="bg1">
                <td>Clan:</td>
                <td colspan="2"><strong><?php echo htmlspecialchars((string)($clandata['name'] ?? ''), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8'); ?></strong></td>
            </tr>
            ...
            <tr class="bg1">
                ...
                <meter min="0" max="100" low="25" high="50" optimum="75" value="<?php
                    echo (float)($clandata['activity'] ?? 0); ?>"></meter>
                </td>
                <td style="width:15%"><?php
                    echo sprintf('%0.2f', (float)($clandata['activity'] ?? 0)).'%';
                ?></td>
            </tr>
            ...
            <tr class="bg2">
                <td>Members:</td>
                <td colspan="2"><?php
                    echo (int)($clandata['nummembers'] ?? 0).
                    " active members (".(int)($totalclanplayers ?? 0)." total)";
                ?></td>
            </tr>
            ...
            <tr class="bg1">
                ...
                <td colspan="2"><strong><?php
                    echo number_format((int)($clandata['avgskill'] ?? 0));
                ?></strong></td>
            </tr>
            ...
            <tr class="bg2">
                <td>Total Kills:</td>
                <td colspan="2"><?php
                    echo number_format((int)($clandata['kills'] ?? 0));
                ?></td>
            </tr>
            ...
            <tr class="bg1">
                <td>Total Deaths:</td>
                <td colspan="2"><?php
                    echo number_format((int)($clandata['deaths'] ?? 0));
                ?></td>
            </tr>
            ...
            <tr class="bg2">
                ...
                <td colspan="2"><?php
                    ...
                    $num_members = (int)($clandata['nummembers'] ?? 0);
                    $avg_kills = ($num_members > 0) ? ((int)($clandata['kills'] ?? 0) / $num_members) : 0;
            ...
            <tr class="bg1">
                ...
                <td colspan="2"><?php
                    $deaths = (int)($clandata['deaths'] ?? 0);
                    if ($deaths != 0)
                    {
                        echo sprintf('<strong>%0.2f</strong>', (int)($clandata['kills'] ?? 0) / $deaths);
                    }
                    else
                    {
                        echo '-';
            ...
            <tr class="bg2">
                ...
                <td colspan="2" style="width:55%;"><?php
                    $con_time = (int)($clandata['connection_time'] ?? 0);
                    if ($con_time > 0) {
                        echo sprintf("%.2f", ((int)($clandata['kills'] ?? 0) / ($con_time / 60)));
                    } else {
                        echo '-';
                    }
                ?></td>
            ...
            <tr class="bg1">
                ...
                <td colspan="2"><?php
                    ...
                    if ($row) {
                        ...
                        $favServerName = htmlspecialchars((string)$row[1], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
                        echo "<a href='hlstats.php?game=$game_url&amp;mode=servers&amp;server_id=$favServerId'> $favServerName </a>";
                    } else {
                        echo '-';
                    }
                ?></td>
            ...
            <tr class="bg2">
                ...
                <td colspan="2"><?php
                    ...
                    $row = $db->fetch_row();
                    $db->free_result();
                    $favMap = ($row) ? (string)$row[0] : '';
                    if ($favMap !== '') {
                        $favMap_safe = htmlspecialchars($favMap, ENT_QUOTES, 'UTF-8');
                        echo "<a href='hlstats.php?game=$game_url&amp;mode=mapinfo&amp;map=" . urlencode($favMap) . "'> $favMap_safe </a>";
                    } else {
                        echo '-';
                    }
                ?></td>
            ...
            <tr class="bg1">
                ...
                <td colspan="2"><?php
                    ...
                    {
                        $fav_weapon = (string)$rowdata[0];
                        $weap_name = htmlspecialchars((string)$rowdata[1], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
                    }
                    ...
                    $image = getImage("/games/$game/weapons/" . strtolower($fav_weapon));
                    if (!$image && !empty($realgame)) {
                        $image = getImage("/games/$realgame/weapons/" . strtolower($fav_weapon));
                    }
                    // check if image exists
                    $fav_weapon_url = urlencode($fav_weapon);
                    $weaponlink = "<a href=\"hlstats.php?mode=weaponinfo&amp;weapon=$fav_weapon_url&amp;game=$game_url\">";

                    if ($image && !empty($image['url'])) {
                        $cellbody = "$weaponlink<img src=\"" . htmlspecialchars((string)$image['url'], ENT_QUOTES, 'UTF-8') . "\" alt=\"$weap_name\" title=\"$weap_name\" style=\"vertical-align:middle;\" />";
                    } else {
                        $cellbody = "$weaponlink<strong> $weap_name</strong>";
                    }
...
<?php
    ...
    $clan_kills_safe = ((int)($clandata['kills'] ?? 0) == 0) ? 1 : (int)$clandata['kills'];
...
<?php
    ...
    if ( isset($g_options['show_google_map']) && $g_options['show_google_map'] == 1 ) {
        include_once(INCLUDE_PATH . '/google_maps.php');
        printMap('clan');
    }
?>