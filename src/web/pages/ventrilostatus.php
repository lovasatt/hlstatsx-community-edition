...
function StrKey( $src, $key, &$res )
{
    $src = (string)$src;
    $key = (string)$key . " ";
    if ( strncasecmp( $src, $key, strlen( $key ) ) !== 0 )
        return false;
...
class CVentriloStatus
{
    ...
    public      $m_cmdpass = '';        // Server password
    public      $m_auth = '';           // Server auth flag
    ...
    {
        if (!isset($this->m_channellist[$idx])) {
            return '';
        }
 ...
 */
function decbin2( $val ) { return str_pad( decbin( (int)$val ), 32, "0", STR_PAD_LEFT ); }
...
class Vent
{
    ...
    */
    ...
    {
        $crc = 0;
        $dtoks = unpack( "c*", (string)$data );         // Note: unpack starts output array index at 1, NOT 0.
        $table = getCRCref();                   // my CRC table reference

        for( $i = 1; $i <= count($dtoks); $i++ ) {
            $crc = $table[ $crc >> 8 ] ^ ($dtoks[$i] & 0xFF) ^ ( smallCast( $crc << 8, 16 ) );
        }
...
class VentRequestPacket extends VentPacket
{
    ...
    {
        $rndupk = unpack( "vx", pack( "S", mt_rand( 1, 65535 )));       // need this in little endian
        $rnd = $rndupk['x'];
    ...
    function encodeData()
    {
        ...
        $chars = unpack( "c*", (string)$this->rawdata );                // 1 indexed array
        $table = getDataEncodeRef();                            // Data table reference
        $encdata = '';

        for( $i = 1; $i <= count( $chars ); $i++ ) {
            $chars[$i] = smallCast( ($chars[$i] & 0xFF) + $table[$a2] + (( $i - 1 ) % 72 ), 8 );
    ...
    function __construct( $cmd, $id, $pass )
    {
        ...
        $this->rawdata = pack( "a16", (string)$pass );                  // the only thing in the data part.
...
class VentResponsePacket extends VentPacket
{
    ...
    */
    function decodeData()
    {
        ...
        for( $i = 1; $i <= count( $chars ); $i++ ) {
            $dec_char = smallCast( $chars[$i] - $table[$a2] - (( $i - 1 ) % 72 ), 8 );
            $a2 = smallCast( $a2 + $a1, 8 );
            $this->rawdata .= chr( $dec_char );
        }
    ...
    */
    function __construct( $packet )
    {
        ...
        if ( !$this->decodeHeader() ) { return; }
        if ( !$this->decodeData() ) { return; }
