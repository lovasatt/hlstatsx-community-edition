...
class teamspeakDisplayClass
{
    ...
    function _setPlayerDisplayName(&$playerInfo) {
        $uStatus = (int)$playerInfo["userstatus"];
        $priv = (int)$playerInfo["privileg"];
        $attr = (int)$playerInfo["attribute"];
	...
	// Determine userlevel (0 = Not server admin, 1 = Server admin)
        $userstatusA = isset($a["userstatus"]) ? (int)$a["userstatus"] : 0;
        $userstatusB = isset($b["userstatus"]) ? (int)$b["userstatus"] : 0;
	...
	if ($serverInfo["queryerror"] != 0) {
	    ...
	    $popupInfo = "Server address: " . htmlspecialchars($settings["serveraddress"]) . (((int)$settings["serverudpport"] != 8767) ? (":" . $settings["serverudpport"]): "");
	    ...
	    $jsTeamspeakId = md5($settings["serveraddress"] . ":" . $settings["serverudpport"]);
            ...
            $safe_address = addslashes($settings["serveraddress"]);
            $safe_forbidden = addslashes($settings["forbiddennicknamechars"]);
		...
		echo("	var password=window.prompt('Enter the teamspeak server password for " . addslashes($serverInfo["serverinfo"]["server_name"]) . "', '');\n");
	    ...
	    $popupInfo = "Server address: " . htmlspecialchars($settings["serveraddress"]) . (((int)$settings["serverudpport"] != 8767) ? (":" . $settings["serverudpport"]): "") . ", Max players: " . $max_players . ", Uptime: " . $this->_formatTime($server_uptime);
