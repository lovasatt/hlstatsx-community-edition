...
*/
    ...
    $flag_in = isset($_GET['flag']) ? (string)$_GET['flag'] : '';
    $flag = valid_request($flag_in, false);
    ...
    $cl_name = str_replace(' ', '&nbsp;', htmlspecialchars($raw_name, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8'));

    // PHP 8 Fix: Check if tag exists (countries might not have tags like clans)
    $raw_tag = isset($clandata['tag']) ? (string)$clandata['tag'] : '';
    $cl_tag  = str_replace(' ', '&nbsp;', htmlspecialchars($raw_tag, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8'));
    $cl_full = trim("$cl_tag $cl_name");
    ...
    pageHeader(
        ...
        array(
            ...
            'Country Rankings' => $scripturl . "?mode=countryclans&amp;game=$game_url",
            'Country Details' => ''
        ),
        $raw_name
    );
            ...
            <table class="data-table">
                ...
                <tr class="bg1">
                    ...
                    <td colspan="2"><?php
                        ...
                        echo '<strong>' . htmlspecialchars((string)$clandata['name'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . '</strong>';
                    ?></td>
                </tr>
                <tr class="bg2">
                ...
                <tr class="bg1">
                    ...
                    <td colspan="2"><?php
                        ...
                        if ($deaths != 0)
                        {
                            echo '<strong>' . sprintf('%.2f', (int)$clandata['kills'] / $deaths) . '</strong>';
                        }
                        else
                        {
                            echo '-';
...
<?php
            ...
            if (file_exists(IMAGE_PATH.'/flags/'.$flag_img_name.'_large.png')) {
                echo '<img src="'.IMAGE_PATH.'/flags/'.$flag_img_name.'_large.png" style="border:0px;" alt="'.htmlspecialchars((string)$flag, ENT_QUOTES, 'UTF-8').'" />';
            } else {
