...
<?php
	...
	$server_id = $rowdata['serverId'];
        // Re-escape inside loop just in case, though it comes from DB
	$game = $rowdata['game'];
	...
	$map_teama_wins = (int)$rowdata['map_ct_wins'];
	$map_teamb_wins = (int)$rowdata['map_ts_wins'];
	$mode = 'playerinfo';
	if (isset($_SERVER['PHP_SELF']) && strpos($_SERVER['PHP_SELF'], 'ingame') !== FALSE)
	{
	    $mode = 'statsme';
	}
...
<?php 
	...
	while (isset($teamdata[$curteam]))
	{
	    ...
	    $team_display_name  = $team_name_str ? htmlspecialchars($team_name_str) : htmlspecialchars(ucfirst(strtolower($team_name_str)));
	    
            if ($team_display_name == '') {
		// Team is not in hlstats_Teams, but is present in the game log
		$team_display_name = $team_code_str ? htmlspecialchars(ucfirst(strtolower($team_code_str))) : '';
	...
	<td style="text-align:left;<?php echo $teamcolor ?>" class="fSmall"><?php
		if (isset($thisplayer))
		{
		    ...
		    if (isset($g_options['countrydata']) && $g_options['countrydata'] == 1)
		    {
                        ...
                        $country_name = isset($thisplayer['cli_country']) ? strtolower($thisplayer['cli_country']) : 'unknown';
...
?>		</td>
	<td style="<?php echo $teamcolor ?>" class="fSmall">
            <?php
		if (isset($thisplayer))
		{
		    $acc = sprintf('%.0f', 0);
	...
	<td style="<?php echo $teamcolor ?>" class="fSmall"><?php
		if (isset($thisplayer))
		{
		    echo sprintf('%.0f', (int)$thisplayer['ping'] / 2);
		}
		else
		{
		    echo '&nbsp;';
...
?>		</td>
	<td style="<?php echo $teamcolor ?>" class="fSmall"><?php
		if (isset($thisplayer))
		{
		    echo number_format((int)$thisplayer['skill']);
		}
		else
		{
		    echo '&nbsp;';
...
?>		</td>
	...
	<td style="<?php echo $bordercolor ?>" class="fSmall"><?php
		if (count($teamdata[$curteam]) > 0)
		{
                    // PHP 8 Fix: Ensure count is not zero
                    $team_count = count($teamdata[$curteam]);
                    $avg_ping = ($team_count > 0) ? $teamdata[$curteam]['teamping'] / $team_count : 0;
		    echo sprintf('%.0f', $avg_ping);
		}
		else
		{
		    echo '&nbsp;';
...
?>		</td>
	...
	<td style="<?php echo $bordercolor ?>" class="fSmall"><?php
		if (count($teamdata[$curteam])>0)
		{
		    echo number_format((int)sprintf('%.0f', $teamdata[$curteam]['teamskill']));
		}
		else
		{
		    echo '&nbsp;';
