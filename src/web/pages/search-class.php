    ...
    class Search
    {
        ...
        function drawForm ($getvars = array(), $searchtypes = -1)
        {
            ...
            if (!is_array($searchtypes))
            {
                ...
                if ($mode !== 'LAN' && !empty($_SESSION['loggedin']) && $acclevel >= 80) {
    ...
    <div class="subblock">
        <form method="get" action="<?php echo $scripturl; ?>">
            <?php
                ...
                {
                    echo '<input type="hidden" name="' . htmlspecialchars((string)$var, ENT_QUOTES, 'UTF-8') . '" value="' . htmlspecialchars((string)$value, ENT_QUOTES, 'UTF-8') . "\" />\n";
                }
            ?>
                    <table class="data-table" style="width:30%;">
                        <tr style="vertical-align:middle;" class="bg1">
                            ...
                            <td style="width:70%;">
                                <input type="text" name="q" size="20" maxlength="128" value="<?php echo htmlspecialchars($this->query, ENT_QUOTES, 'UTF-8'); ?>" style="width:300px;" />
                            </td>
                        </tr>
                        <tr style="vertical-align:middle;" class="bg1">
...
<?php
            ...
            // Security: Escape game variable for queries
            $game_esc = $db->escape($this->game);
            ...
            if ($this->type == 'player')
            {
                ...
                if ($this->game)
                    $andgame = "AND hlstats_Players.game='" . $game_esc . "'";
                else
                    $andgame = '';
                $result = $db->query
                ("
            ...
            elseif ($this->type == 'uniqueid')
            {
                ...
                if ($this->game)
                    $andgame = "AND hlstats_PlayerUniqueIds.game='" . $game_esc . "'";
                else
                    $andgame = '';
                $result = $db->query
                ("
            ...
            {
                $acclevel = (int)($_SESSION['acclevel'] ?? 0);
                if (empty($_SESSION['loggedin']) || $acclevel < 80) {
                    die ("Access denied!");
                }
                $table = new Table
                (
            ...
            elseif ($this->type == 'clan')
            {
                ...
                if ($this->game)
                    $andgame = "AND hlstats_Clans.game='" . $game_esc . "'";
                else
                    $andgame = "";
                $result = $db->query
                ("
...
?>
    ...
    <div class="subblock" style="text-align:center;">
        Search results: <strong><?php echo isset($numitems) ? (int)$numitems : 0; ?></strong> items matching
    </div>
</div>
<?php
        }
