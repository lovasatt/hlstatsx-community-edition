...
// Search Class
    // PHP 8 Fix: Allow dynamic properties if needed (though we define them)
...
<div class="block">
	...
	<form method="get" action="<?php echo htmlspecialchars($g_options['scripturl']); ?>">
		...
		{
		    echo '<input type="hidden" name="'.htmlspecialchars((string)$var, ENT_QUOTES).'" value="'.htmlspecialchars((string)$value, ENT_QUOTES)."\" />\n";
		}
	    ?>
...
?>
	...
	Search results: <strong><?php echo isset($numitems) ? $numitems : 0; ?></strong> items matching
    </div>
</div>
<?php
	}
