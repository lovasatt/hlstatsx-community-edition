...
<?php
    if (isset($_SESSION['nojs']) && (int)$_SESSION['nojs'] === 1) {
    ...
    $version_out = htmlspecialchars((string)($g_options['version'] ?? 'Unknown'), ENT_QUOTES, 'UTF-8');
    echo 'Generated in real-time by <a href="http://www.hlxce.com" target="_blank">HLstatsX Community Edition ' . $version_out . '</a>';

    if (isset($g_options['showqueries']) && (int)$g_options['showqueries'] === 1) {
        $query_count = (int)($db->querycount ?? 0);
        echo '
            <br />
...
This product includes GeoLite2 data created by MaxMind, <br />
available from <a href="https://www.maxmind.com" target="_blank" rel="noopener noreferrer">https://www.maxmind.com</a>.
...
<?php
    ...
    $show_map = (isset($g_options["show_google_map"]) && (int)$g_options["show_google_map"] === 1);
    $is_content_mode = (isset($mode) && (string)$mode === "contents");
    $has_redirect = (isset($redirect_to_game) && (int)$redirect_to_game > 0);
