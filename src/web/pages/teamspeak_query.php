...
function defaultInfo($ip,$tPort,$port)
{
    ...
    if($fp) {
        ...
        $name       = preg_match('/server_name=([^\r\n]*)/i', $out, $m) ? convertCharset(trim($m[1])) : 'Unknown';
        $os         = preg_match('/server_platform=([^\r\n]*)/i', $out, $m) ? convertCharset(trim($m[1])) : 'Unknown';
        $tsType     = preg_match('/server_clan_server=([^\r\n]*)/i', $out, $m) ? trim($m[1]) : '0';
        $welcomeMsg = preg_match('/server_welcomemessage=([^\r\n]*)/i', $out, $m) ? convertCharset(trim($m[1])) : '';

        if (isset($tsType[0]) && $tsType[0] == '1') {
...
function channelInfo($ip,$tPort,$port,$cID)
{
    ...
    {
        $innArray = $cArray[$i];
        if(isset($innArray[0]) && $innArray[0]==$cID)
        {
...
function convertCharset($text)
{
    ...
    if (function_exists('iconv')) {
        return (string)@iconv('Windows-1252', 'UTF-8//IGNORE', $text);
    }
    return $text;
}
?>