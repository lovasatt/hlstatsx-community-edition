...
*/
    ...
    if (!$player && $uniqueid) {
        if (!$game) {
            $redirect_url = ($g_options['scripturl'] ?? '') . "?mode=search&st=uniqueid&q=" . urlencode((string)$uniqueid);
        ...
        $uniqueid_esc = $db->escape((string)$uniqueid);
        ...
        $db->query("
            ...
            WHERE
                ...
                AND hlstats_PlayerUniqueIds.game = '$game_esc'
        ");


        if ($db->num_rows() > 1) {
            $redirect_url = ($g_options['scripturl'] ?? '') . "?mode=search&st=uniqueid&q=" . urlencode((string)$uniqueid) . "&game=" . urlencode((string)$game);
            header("Location: $redirect_url");
            exit;
        } elseif ($db->num_rows() < 1) {
            error("No players found matching uniqueId '" . htmlspecialchars((string)$uniqueid, ENT_QUOTES, 'UTF-8') . "'.");
        } else {
...
// May get moved in the future
    ...
    $db->query("
        SELECT
            COUNT(killerId) AS total_kills,
            SUM(IF(headshot = 1, 1, 0)) AS total_headshots
        FROM
    ...
    if (isset($_GET['type']) && $_GET['type'] == 'ajax') {
        ...
        // Fix: Allow underscores ([^a-z_]) for composite tab names like 'general_aliases'
