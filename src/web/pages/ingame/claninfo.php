...
*/
    ...
    $cl_raw_name = (string)($clandata['name'] ?? '');
    $cl_name = preg_replace('/ /', '&nbsp;', htmlspecialchars($cl_raw_name, ENT_QUOTES, 'UTF-8'));
    $cl_tag  = preg_replace('/ /', '&nbsp;', htmlspecialchars((string)($clandata['tag'] ?? ''), ENT_QUOTES, 'UTF-8'));
    $cl_full = "$cl_tag $cl_name";

    // Check if game exists in data
    $game = (string)($clandata['game'] ?? $game ?? '');
    ...
    pageHeader(
        array($gamename, 'Clan Details', $cl_name),
        array(
            $gamename => ($g_options['scripturl'] ?? 'hlstats.php') . "?game=" . urlencode($game),
            'Clan Rankings' => ($g_options['scripturl'] ?? 'hlstats.php') . "?mode=clans&game=" . urlencode($game),
            'Clan Details' => ''
        ),
        $cl_name
    );
    ...
    <table class="data-table">
        ...
        <tr class="bg1">
            ...
            <td colspan="2" class="fSmall"><?php
                echo '<strong>'.htmlspecialchars((string)($clandata['name'] ?? ''), ENT_QUOTES, 'UTF-8').'</strong>';
            ?></td>
        </tr>
        <tr class="bg2">
        ...
        <tr class="bg1">
            ...
            <td colspan="2" class="fSmall"><?php
                echo number_format((int)($clandata['nummembers'] ?? 0));
            ?></td>
        </tr>
        <tr class="bg2">
        ...
        <tr class="bg1">
            ...
            <td style="width:40%;">
                <?php
                    $width = sprintf('%d%%', (float)($clandata['activity'] ?? 0) + 0.5);
                ?>
                <meter min="0" max="100" low="25" high="50" optimum="75"
                value="<?php echo (float)($clandata['activity'] ?? 0) ?>"></meter>
        </td>
        ...
        <tr class="bg2">
            ...
            <td colspan="2" class="fSmall"><?php
                ...
                if ($deaths != 0)
                {
                    printf('%0.2f', (int)($clandata['kills'] ?? 0) / $deaths);
                }
                else
                {
                    echo '-';
        ...
        <tr class="bg1">
            ...
            <td colspan="2" class="fSmall"><?php
                echo timestamp_to_str((int)($clandata['connection_time'] ?? 0));
            ?></td>
        </tr>
    </table>
<?php
    ...
    $clan_kills = (int)($clandata["kills"] ?? 0);
