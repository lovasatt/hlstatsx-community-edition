...
*/
    ...
    if (!$player && $uniqueid) {
        if (!$game) {
            $redirect = ($g_options['scripturl'] ?? 'hlstats.php') . "?mode=search&st=uniqueid&q=" . urlencode($uniqueid);
        ...
        if ($db->num_rows() > 1) {
            $redirect = ($g_options['scripturl'] ?? 'hlstats.php') . "?mode=search&st=uniqueid&q=" . urlencode($uniqueid) . "&game=" . urlencode($game);
            header("Location: $redirect");
            exit;
