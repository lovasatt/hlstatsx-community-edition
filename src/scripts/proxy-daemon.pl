...
sub assignDaemon
{
        ...
        if (defined($$srv_list{'rr-next'})) {
                $next = int($$srv_list{'rr-next'});
        } else {
                $next = 0;
        }

        my $max = scalar(keys %$daemon);
        return if ($max == 0);
...
sub checkHeartbeat
{
        ...
        foreach my $key (keys(%$daemon)) {
                next unless (defined($$daemon{$key}{'ip'}) && defined($$daemon{$key}{'port'}));
                my $socket = IO::Socket::INET->new(     Proto=>"udp",
                                                        PeerHost=>$$daemon{$key}{'ip'},
                                                        PeerPort=>$$daemon{$key}{'port'}
                                                );
                if ($socket) {
                        my $packet = "C;HEARTBEAT;";
                        $socket->send("PROXY Key=$proxy_key PROXY $packet");

                        if (IO::Select->new($socket)->can_read(4)) {
                                my $msg = "";
...
#
# string returnServerList(hash srv_list)
#
...
sub returnServerList
{
        ...
        for my $ip (keys(%$srv_list)) {
                next if ($ip eq 'rr-next');
                for my $port (keys(%{$$srv_list{$ip}})) {
                        $msg .= "$ip:$port -> " . $$srv_list{$ip}{$port}{'dest_ip'} . ":" . $$srv_list{$ip}{$port}{'dest_port'} . "\n";
                }
        }
...
sub reloadDaemon
{
        ...
        foreach my $key (keys(%$daemon)) {
                if ($$daemon{$key}{'curstate'} eq "up") {
                        ...
                        $msg .= &theTime() . "Sending RELOAD packet to " . $$daemon{$key}{'ip'} . ":" . $$daemon{$key}{'port'} . "\n";
...
# Connect to mysql DB to get required settings
&doConnect();

my $proxy_key = &getProxyKey();

# Get the daemons you will use
my $query = "SELECT `value` FROM hlstats_Options WHERE `keyname` = 'Proxy_Daemons'";
my $result = &doQuery($query);
...
# It went ok, lets start receive messages...
&printEvent("DAEMON", "HLstatsX:CE Proxy Daemon up and running on port: $proxy_port, key: $proxy_key", 1);
...
while ($server->recv($datagram, 8192, $flags)) {
        my $control = 0;
        my $msg = '';
        ...
        if ($datagram =~ /PROXY Key=(.+) (.*)PROXY (.+)/) {
                if ($proxy_key eq $1) {
                        if ($3 =~ /C;HEARTBEAT;/) {
                                ...
                                $msg = "ServerList\n$msg";
                                &printEvent("CONTROL", "Sending Serverlist to $ipaddr:$ipport", 1);
                                &printEvent("CONTROL", $msg, 1) if ($g_debug > 1);
                        } elsif ($3 =~ /C;RELOAD;/) {
                                $msg = &reloadDaemon(\%daemon, $proxy_key);
                        }
                } else {
        ...
        if (defined($srv_list{$ipaddr}{$ipport})) {
                foreach my $key (keys %daemon) {
                        if ($srv_list{$ipaddr}{$ipport}{'dest_ip'} eq $daemon{$key}{'ip'} && $srv_list{$ipaddr}{$ipport}{'dest_port'} eq $daemon{$key}{'port'}) {
                                if ($daemon{$key}{'curstate'} eq "up" && $daemon{$key}{'oldstate'} eq "down") {
                                        %srv_list = ();
                                } elsif ($daemon{$key}{'curstate'} eq "down") {
                                        delete $srv_list{$ipaddr}{$ipport};
                                        &assignDaemon($ipaddr, $ipport, \%daemon, \%srv_list);
                                        &printEvent("BALANCE", "Re-Assigned daemon $$srv_list{$ipaddr}{$ipport}{'dest_ip'}:$$srv_list{$ipaddr}{$ipport}{'dest_port'} to $ipaddr:$ipport", 1);
                                }
                        }
                }
        } else {
                delete $srv_list{$ipaddr}{$ipport};
                &assignDaemon($ipaddr, $ipport, \%daemon, \%srv_list);
                &printEvent("BALANCE", "Assigned daemon $$srv_list{$ipaddr}{$ipport}{'dest_ip'}:$$srv_list{$ipaddr}{$ipport}{'dest_port'} to $ipaddr:$ipport", 1);
        }
