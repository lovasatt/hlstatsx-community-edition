...
sub new
{
        ...
        if ($::g_stdin == 0 && $self->{playerid} > 0 && ($self->{is_bot} == 0 || $::g_servers{$self->{server}}->{ignore_bots} == 0)) {
...
sub set
{
        ...
        if (defined($self->{$key}))
        {
                ...
                elsif ($key eq "skill" && ($self->{userid} < 0 || $self->{is_bot} == 1))
                {
                        return $self->{skill};
                }
                else
...
sub increment
{
        ...
        if ($key eq "skill" && ($self->{userid} < 0 || $self->{is_bot} == 1)) {
                return $self->{skill};
        }
...
{
        my ($self) = @_;
        my $flag = ($self->{is_bot} == 1 && ($self->{flag} eq "" || !defined($self->{flag}))) ? "BOT" : $self->{flag};
        my $addr = ($self->{is_bot} == 1 && ($self->{address} eq "" || !defined($self->{address}))) ? "127.0.0.1" : $self->{address};
        my $steamid = ($self->{is_bot} == 1 && ($self->{plain_uniqueid} eq "" || !defined($self->{plain_uniqueid}))) ? "BOT" : $self->{plain_uniqueid};
        ...
        ";
        my @vals = ($self->{playerid}, $self->{server_id}, $addr, $steamid,
                $self->{name}, $self->{team}, $self->{ping}, $self->{connect_time}, $self->{skill}, $flag);
...
sub flushDB
{
    ...
    my $server_address = $self->{server};
        ...
        if ($::g_stdin == 0 && $playerid > 0 && ($self->{is_bot} == 0 || $::g_servers{$server_address}->{ignore_bots} == 0)) {
                # Update live stats
                my $addr = ($is_bot == 1 && ($address eq "" || !defined($address))) ? "127.0.0.1" : $address;
                my $sid  = ($is_bot == 1 && ($steamid eq "" || !defined($steamid))) ? "BOT" : $steamid;
