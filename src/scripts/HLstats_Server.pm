...
sub rcon_getplayers
{
    my ($self) = @_;
    my %players = ();
    my $res = $self->dorcon("status");

    return %players unless (defined($res) && $res ne "");

    my @lines = split(/\r?\n/, $res);

    foreach my $line (@lines)
    {
        # 1. Clean line: strip leading engine prefixes and whitespace
        $line =~ s/^\s*(?:\[(?:Client|Server|EngineServiceManager)\]\s*)?//i;
        $line =~ s/^\s+//;
        $line =~ s/\s+$//;

        # 2. Skip all Source 2 / Source 1 / GoldSrc headers and metadata
        ...
        # 3. Skip CS2 65535 unconnected/challenging ghost sockets
        ...
        # ---------------------------------------------------------------------
        # CS2 Bots (Format: 1 BOT 0 0 active 0 'BotName')
        # ---------------------------------------------------------------------
        if ($line =~ /^#?\s*(\d+)\s+BOT\s+(\d+)\s+\d+\s+\S+\s+(?:.*?)['"](.*?)['"]\s*$/i)
        {
            $userid   = $1;
            $ping     = $2;
            $name     = $3;
        ...
        # ---------------------------------------------------------------------
        # CS2 Humans (Format: 2 04:12 25 0 active 786432 188.142.158.236:50231 'Name')
        # Handles 0unknown, 786432unknown, H:MM:SS time, 'Name' with single quotes
        # ---------------------------------------------------------------------
        elsif ($line =~ /^#?\s*(\d+)\s+\S+\s+(\d+)\s+\d+\s+\S+\s+(.*?)\s*['"](.+?)['"]\s*$/i)
        {
            $userid = $1;
            $ping   = $2;
            my $middle = $3;
        ...
        # ---------------------------------------------------------------------
        # Source 1 Engine (TF2, CS:GO, CS:S, L4D2, DoD:S)
        # Format: # 2 1 "Player" STEAM_0:0:12345 04:12 25 0 active 192.168.1.50:27005
        # ---------------------------------------------------------------------
        elsif ($line =~ /^#\s*(\d+)\s+(?:\d+\s+)?"?([^"]+)"?\s+((?:\[U:\d+:\d+\]|STEAM_[0-9:]+|VALVE_[0-9:]+|BOT|UNKNOWN|[0-9]+))\s+\S+\s+(\d+)\s+\d+\s+[a-zA-Z]+\s+(?:\d+\s+)?([0-9\.:]+)?/i)
        {
            $userid   = $1;
            $name     = $2;
            $uniqueid = $3;
            $ping     = $4;
            $address  = $5 if defined($5);
        }
        # ---------------------------------------------------------------------
        # GoldSrc / CS 1.6 / HL1 / TFC / DoD
        # Format: # 1 "Player" 1234 STEAM_0:0:12345 0 00:10 50 0 192.168.1.50:27005
        # ---------------------------------------------------------------------
        elsif ($line =~ /^#?\s*\d+\s+"([^"]+)"\s+(\d+)\s+(STEAM_[0-9:]+|VALVE_[0-9:]+|BOT|UNKNOWN)\s+\d+\s+\S+\s+(\d+)\s+\d+\s+([^:\s]+:\d+)?/i)
        {
            $name     = $1;
            $userid   = $2;
            $uniqueid = $3;
        ...
        # Generic Fallback
        # ---------------------------------------------------------------------
        elsif ($line =~ /^#\s*(\d+)\s+["'](.+?)["'](?:\s+([^\s]+))?/i)
        {
            $userid   = $1;
            $name     = $2;
            $uniqueid = $3 if defined($3);
        }

        # Clean quotes and extra whitespace from name
        if ($name ne "") {
            $name =~ s/^['"]+|['"]+$//g;
            $name =~ s/^\s+|\s+$//g;
        }
        ...
        # Multi-Key Indexing: Store by UserID, Name, UniqueID and Address
        if ($userid ne "") {
            $players{$userid} = $pdata;
            $players{int($userid)} = $pdata;
        }
        ...
        if ($uniqueid ne "" && $uniqueid ne "BOT") {
            $players{$uniqueid} = $pdata;
            my $norm_uid = $uniqueid;
            $norm_uid =~ s!\[U:1:(\d+)\]!'STEAM_0:'.($1 % 2).':'.int($1 / 2)!egi;
            $norm_uid =~ s/^STEAM_[0-9]+?\://i;
            $players{$norm_uid} = $pdata if ($norm_uid ne "");
        }

        if ($address ne "" && $address ne "none" && $address ne "loopback") {
            $players{$address} = $pdata;
            if ($address =~ /^([\d\.]+):/) {
                $players{$1} = $pdata;
            }
        }
        ...
        if ($last_timestamp > 0)
        {
            if ($last_timestamp+299 < $new_timestamp)
            {
                ...
                if ($self->{play_game} == CS2()) {
                    my $status_json = $self->dorcon("status_json");
                    my @lines = split(/[\r\n]+/, $status_json);
                    foreach my $line (@lines) {
                        if ($line =~ /^\s*"map": "([^"]+)".*/) {
                            $map = $1;
...
# Update Player Pings (CS2, Source 1, GoldSrc & Multi-Key Indexing)
#
sub update_players_pings
{
        ...
        {
                &::printNotice("(IGNORED) NOTMINPLAYERS: Update_player_pings (Active: $self->{num_trackable_players} / Min: $self->{minplayers})");
        }
        else
        {
                ...
                {
                        # Only track human players (skip bots)
                        if ($player && $player->{is_bot} == 0) {
                                my $uniqueid = $player->{uniqueid} // "";
                                my $name     = $player->{name} // "";
                                my $address  = $player->{address} // "";
                                my $userid   = $player->{userid} // "";

                                # Multi-key lookup: UniqueID -> Name -> Address -> UserID
                                my $pinfo = $players{$uniqueid}
                                         || $players{$name}
                                         || $players{$address}
                                         || (defined($userid) && $userid ne "" ? $players{$userid} : undef);

                                if ($pinfo) {
                                        my $ping = int($pinfo->{"Ping"} // 0);
                                        $player->set("ping", $ping);

                                        # Record latency for LAN testing (0 ms) and standard internet play
        ...
        elsif (($::g_stdin == 0) && ($self->{ba_enabled} > 0))
        {
                ...
                if ($self->{ba_map_rounds} >= 2)
                {
                        ...
                        elsif  ($ts_count + 1 < $ct_count)
                        {
                                ...
                                {
                                        @ct_players = sort { $a->[6] <=> $b->[6]} @ct_players;
                                }
                        ...
                        if (($action_done == 0) && ($self->{ba_last_swap} == 0) && ($self->{ba_map_rounds} >= 7) && ($self->{ba_player_switch} == 0))
                        {
                                if ($ct_wins < 2)
                                {
                                ...
                                elsif ($ts_wins < 2)
                                {
                                        ...
                                        else
                                        {
                                                ...
                                                my $ts_found = 0;
                                                @ts_players = sort { $a->[6] <=> $b->[6]} @ts_players;
                                                foreach my $entry (@ts_players)
                                                {
                                                        if ($ts_found == 0)
                                                        {
